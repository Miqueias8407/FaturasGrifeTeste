...
@st.cache_data
def load_data():
    # Atualize o caminho abaixo para o diretório onde estão suas planilhas
    path = r"C:\Users\miqueias.esteves\Documents\PYTHON\ProjStreamFat\Rel_441\*.xlsx"  # Arquivos Excel na pasta atual
...
with col5:
    ...
    if (end_date - start_date).days <= 31:  # Se o intervalo for menor que 1 mês, agrupar por dia
        filtered_df['Periodo'] = filtered_df['Vencimento'].dt.strftime('%d/%m')
        title = 'Faturas por Dia'
    elif (end_date - start_date).days <= 365:  # Se o intervalo for menor que 1 ano, agrupar por mês
...
# Novo gráfico: Top clientes por valor
st.markdown("---")
st.subheader("Top 10 Clientes")

# Agrupar por cliente e calcular o valor total, ordenar do maior para o menor
top_clientes = filtered_df.groupby('Cliente')['Valor'].sum().sort_values(ascending=False).head(10)

# Formatar os valores para exibição
valores_formatados = [format_brl(val) for val in top_clientes.values]
...
fig_clientes = go.Figure(go.Bar(
    ...
    hovertemplate='<b>%{y}</b><br>Valor Total: %{text}<extra></extra>'
))
...
fig_clientes.update_layout(
    ...
    margin=dict(l=150, r=50, b=50, t=50, pad=5)
)
